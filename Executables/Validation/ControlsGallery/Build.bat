@echo off
REM ============================================================================
REM  ControlsGallery\Build.bat - build ControlsGallery.exe, a STANDALONE UI-only
REM  validation window that showcases every Interface control from
REM  ControlsPreview.html.
REM
REM  Win32 + Direct3D 11 host. It links ONLY EngineContext.lib (the pillar that
REM  carries the Interface controls + theme) + the vendored ImGui core it
REM  compiles itself (core + Win32/DX11 backends) + system D3D11. It does NOT
REM  touch Graphics / Platform / Vulkan - the controls draw on the ImGui draw
REM  list, so no renderer pillar is needed. Validation hosts keep the proven
...
REM  EngineContext carries the Interface controls + theme this app draws with.
...
for %%U in (ControlsGalleryPanel ControlsGalleryHost) do (
    echo [compile] %%U.cpp
