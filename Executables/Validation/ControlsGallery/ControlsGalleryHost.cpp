/*==============================================================================================================================================
                                                            CONTROLSGALLERYHOST.CPP
==============================================================================================================================================*/
// 🧩 The whole standalone window for the UI-only controls gallery. Win32 + Direct3D 11 host (both backends vendored in ExternalPackages/imgui,
//    so NOTHING outside imgui + EngineContext.lib is linked — this app is fully decoupled from Editor / TexturePaint / TextureBake / etc.). Each
//    frame it resolves the active theme, mirrors it into ImGui's style, and draws ConstructControlsGalleryPanel inside one docked window.
//    Deliberately minimal: no RootSystem, no ApplicationConfiguration, no engine spine — just enough to see the real controls render. Validation
//    hosts keep the proven D3D11 backend for quick eyeballing; the editors run on the native Vulkan substrate.
...
int main(int, char**)
{
    ...
    WNDCLASSEXW WindowClass = { sizeof(WindowClass), CS_CLASSDC, WindowProc, 0L, 0L,
                                ...
                                L"FrontierControlsGallery", nullptr };
    ::RegisterClassExW(&WindowClass);
    HWND Window = ::CreateWindowW(WindowClass.lpszClassName, L"Frontier - Controls Gallery",
    ...
    while (Running)
    {
        ...
        // 📝 One full-viewport window hosting the gallery so it reads like a real property column.
        ...
        if (ImGui::Begin("Controls Gallery", nullptr, HostFlags))
        {
            ConstructControlsGalleryPanel(Theme, State);
        }
        ImGui::End();
        ImGui::PopStyleColor();
