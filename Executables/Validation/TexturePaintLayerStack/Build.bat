@echo off
REM ============================================================================
REM  TexturePaintLayerStack\Build.bat - build TexturePaintLayerStack.exe: a
REM  standalone Vulkan validation host for the texture-paint layer stack rail.
REM  Opens one native Vulkan window + ImGui interface, brings up the
REM  SvgIconRegistry against the host, registers the global "g-" + paint "paint-"
REM  icon tiers, seeds the sample document, and drives ConstructLayerStackPanel
REM  each frame until the window closes.
REM
REM  The rail (LayerStackPanel + IconPackPaint) lives in EngineContext.lib; the
REM  Vulkan host + ImGui interface live in Graphics.lib; the window + surface +
REM  relay live in Platform.lib. So this app rebuilds the shared pillar libs
REM  first via their own Build.bat, then compiles ONLY its host unit and links
REM  EngineContext + Graphics + Platform plus thorvg + vulkan-1.lib + the OS
REM  libs. It draws NO renderer passes, so NO shaders are staged.
REM
...
REM  EngineContext (the rail panel + icon tiers + vendored ImGui core).
...
REM  EngineContext.lib) calls into for the paint + global icon glyphs.
