@echo off
REM ============================================================================
REM  SurfelValidation\Build.bat - build + run SurfelValidation.exe: the exit
REM  gate for Phase-1 surfel GI (pool + cascaded hash grid + slotting + prefix
REM  sum + lifecycle). It drives the SHIPPED submissions on a REAL headless
REM  device - SurfelPool, SurfelGridSlotting (which owns SurfelPrefixSum) - and
REM  judges every readback against a CPU oracle that mirrors SurfelGrid.glsl but
REM  shares no code with the shaders. So it links Graphics.lib (the surfel
REM  submissions + the Vulkan host), EngineContext.lib, AuthoringGeometry.lib and
REM  vulkan-1.lib.
REM
REM  It creates NO WINDOW: InitializeVulkanHost passes a zero extension count and
REM  gets a headless compute device, so Platform.lib is NOT linked here.
REM
REM  SHADER STEP REQUIRED. This gate DISPATCHES the real slotting + prefix passes,
REM  so a stale .spv would have it validate the previous revision of a shader and
REM  report a pass. ShaderPlan.ps1 glob-discovers every .comp under Internal.
REM
