...
//    the per-classification icon tints. All of it lives in the app-local namespace SketchOutlinerValidation so its RecordEntry / RecordToken never
//    collide with the pillar's Frontier::RecordEntry.
...
namespace
{
    ...
    // 📝 Draw the row's icon at Origin: prefer the real uploaded SVG texture resolved from the registry by IconKey. The CAD glyphs are authored in
    //    full house colour, so the texture is drawn with a WHITE tint (ImGui multiplies the texture by the tint — a coloured tint would darken and
    //    desaturate the already-coloured glyph, which is what made these read washed). Only TintColor's ALPHA is carried through, so a concealed row
    //    (which arrives with a lowered alpha) still fades without shifting hue. The procedural fallback keeps the full TintColor since its strokes
                       ...
                       ImU32                            TintColor)
    {
        ...
        if (Texture != 0)
        {
            const ImU32 ImageTint = IM_COL32(0xff, 0xff, 0xff, (TintColor >> IM_COL32_A_SHIFT) & 0xFF);
...
namespace
{
    ...
    void ConstructContextMenu(SketchOutlinerState& State, const Frontier::SvgIconRegistry* Registry)
    {
        ...
        if (BeginDropdownPopup("##sketch-ctx", 210.0f))
        {
    ...
    void ConstructAddMenu(SketchOutlinerState& State, const Frontier::SvgIconRegistry* Registry)
    {
        ...
        if (BeginDropdownPopup("##sketch-add", 230.0f))
        {
    ...
    void ConstructFilterMenu(SketchOutlinerState& State, const Frontier::SvgIconRegistry* Registry)
    {
        ...
        if (BeginDropdownPopup("##sketch-filter", 230.0f))
        {
