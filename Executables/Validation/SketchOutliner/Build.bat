@echo off
REM ============================================================================
REM  SketchOutliner\Build.bat - build SketchOutlinerValidation.exe: a standalone
REM  Vulkan validation host for the CAD outliner. Opens one native Vulkan window +
REM  ImGui interface, brings up the SvgIconRegistry against the host, registers
REM  both icon tiers, builds the canonical SketchDocument fixture, and drives the
REM  SketchOutlinerPanel each frame until the window closes.
REM
REM  The outliner (document, presentation, panel, icons) all live in
REM  EngineContext.lib; the Vulkan host + ImGui interface live in Graphics.lib; the
REM  window + surface + relay live in Platform.lib. So this app rebuilds the shared
REM  pillar libs first via their own Build.bat, then compiles ONLY its host unit and
REM  links EngineContext + Graphics + Platform plus vulkan-1.lib + the OS libs. It
REM  draws NO renderer passes, so NO shaders are staged. It writes its OWN exe under
REM  Binaries\Validation (alongside SceneDirectory.exe) and never touches it.
REM
...
REM  EngineContext (outliner document/presentation/panel/icons + vendored ImGui core).
...
REM --- Compile this app's own host unit ---------------------------------------
...
REM  EngineContext.lib) calls into for the icon glyphs. This is the first exe to
REM  reference the icon registry, so the first to need thorvg on the link line.
