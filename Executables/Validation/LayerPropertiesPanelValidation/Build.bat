@echo off
REM ============================================================================
REM  LayerPropertiesPanelValidation\Build.bat - build
REM  LayerPropertiesPanelValidation.exe, a STANDALONE UI-only validation window
REM  showing the paint-layer MASK property column (LayerPropertiesPanel).
REM
REM  Win32 + Direct3D 11 host, same shape as ControlsGallery so the two windows
REM  are directly comparable. It links ONLY EngineContext.lib (the pillar that
REM  carries the Interface controls + theme + the panel itself) plus the vendored
REM  ImGui core it compiles (core + Win32/DX11 backends) and system D3D11. It
REM  does NOT touch Graphics / Platform / Vulkan.
REM
...
set "NAME=LayerPropertiesPanelValidation"
...
REM  EngineContext carries the Interface controls + theme + LayerPropertiesPanel.
...
REM --- Compile this app own sources every time (they are the files under edit) --
set "OBJRSP=%OBJ%\link_objs.rsp"
if exist "%OBJRSP%" del /Q "%OBJRSP%"
for %%U in (LayerPropertiesPanelValidationHost) do (
    echo [compile] %%U.cpp
