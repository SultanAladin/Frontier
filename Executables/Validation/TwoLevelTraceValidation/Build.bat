@echo off
REM ============================================================================
REM  TwoLevelTraceValidation\Build.bat - build + run TwoLevelTraceValidation.exe:
REM  the exit gate for TwoLevelTrace.glsl, the ray walk over BOTH acceleration
REM  levels. It drives the WHOLE four-dispatch TLAS chain on a REAL DEVICE -
REM  reduce, Morton, radix sort, tree build, refit - then traces a ray set
REM  against it and judges every hit against a CPU brute-force oracle. So it
REM  links Graphics.lib (all four submissions + the Vulkan host), plus
REM  EngineContext.lib, AuthoringGeometry.lib (BuildGeometryTree) and vulkan-1.lib.
REM
REM  It creates NO WINDOW: InitializeVulkanHost never creates or queries a
REM  surface, so the entry passes a zero extension count and gets a headless
REM  compute device. That is why Platform.lib is NOT linked here.
REM
REM  SHADER STEP REQUIRED. This gate DISPATCHES, so a stale .spv would have it
REM  validate the previous revision of the shader and report a pass.
REM  ShaderPlan.ps1 glob-discovers every .comp under Internal, so no table edit
REM  is needed for the two new modules.
REM
...
set "NAME=TwoLevelTraceValidation"
...
REM  A stale .spv is the silent failure this step exists to prevent: the gate
REM  would exercise the previous revision of the shader and report PASS.
...
REM  CAUTION: FRONTIER_POLYGON_AUTHORING is ABI-AFFECTING - it adds
REM  RenderExtension members, so it MUST match Internal\Graphics\Build.bat's
REM  DEFINES even though this gate never touches RenderExtension, or the struct
REM  layout disagrees across the Graphics.lib boundary.
