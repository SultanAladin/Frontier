@echo off
REM ============================================================================
REM  PaintToolValidation\Build.bat - build PaintToolValidation.exe: a standalone
REM  Vulkan validation host for the two-slide texture-paint tool card ported from
REM  Documentation\Prototypes\PaintToolMenu.html. Opens one native Vulkan window +
REM  ImGui interface, brings up the SvgIconRegistry against the host, registers the
REM  global icon pack plus this app's OWN paint pack, and drives the panel each
REM  frame until the window closes.
REM
REM  Unlike ModellingToolValidation, this app owns its card components outright -
REM  the whole paint card (shell, rail, grid, preview, options), its generated
REM  catalogue + icon pack, and its non-square strip store are all local .cpp here,
REM  NOT in EngineContext.lib. That is deliberate: the paint card needs a
REM  rectangular raster and a per-crop key, neither of which the shared
REM  SvgIconRegistry does, so nothing it needs can perturb the modelling/drafting
REM  cards that already ship. It still links the three pillars for the window, the
REM  Vulkan host, the theme, and the registry itself.
REM
...
REM  EngineContext (theme + SvgIconRegistry + SvgRasterizer + vendored ImGui core).
...
REM  thorvg's own include is needed HERE (unlike the modelling app) because
REM  PaintIconStore.cpp calls tvg:: directly for its rectangular raster.
...
REM  PaintIconStore.cpp calls tvg:: directly and links the STATIC thorvg.lib, so without
REM  it every call goes through an import thunk and the linker emits LNK4217 per symbol.
...
REM --- Compile this app's own units (card components + generated tables + host) -
...
REM  SvgRasterizer.obj and this app's PaintIconStore.obj call into it.
