@echo off
REM ============================================================================
REM  VolumeBoundsValidation\Build.bat - build + run VolumeBoundsValidation.exe: the
REM  exit gate for the LBVH scene-bounds reduce. Unlike the CPU-only validation tools this
REM  one needs a REAL DEVICE - it runs the bounds reduce dispatch and judges
REM  the readback against a CPU min/max - so it links Graphics.lib (the sort
...
set "NAME=VolumeBoundsValidation"
...
REM  Graphics carries VolumeBoundsSubmission + VulkanHost; EngineContext carries the
