/*==============================================================================================================================================
                                                    MODELLINGTOOLVALIDATIONHOST.CPP
==============================================================================================================================================*/
// 🧩 Standalone Vulkan validation host for the two-slide modelling tool card ported from Documentation/Prototypes/ModellingToolMenu.html. It stands
//    up the shared Vulkan spine (PlatformWindow + VulkanHost + presentation surface + VulkanImguiInterface + the Win32 ImGui relay), then — once the
//    ImGui Vulkan backend and its font texture exist — brings up the SvgIconRegistry against the same host and registers the global pack plus the
//    card's own ToolMenu pack. Each frame it drives ConstructModellingToolPanel inside one docked-full window, so a human can exercise every stratum,
//    the rail's filtering, the gated tiles, the carousel slide and all six parameter widget kinds against the prototype side by side.
...
int main(int ArgumentCount, char** ArgumentValues)
{
    ...
    if (!InitializePlatformWindow(Window, "Frontier \xE2\x80\x94 Modelling Tool Card Validation", 1280, 900))
    {
        fprintf(stderr, "[toolcard-validation] window creation failed\n");
        return 1;
    }
    ...
    {
        fprintf(stderr, "[toolcard-validation] surface creation failed\n");
    ...
    //    same configuration, so the authoring column and the card under review scale together.
    ...
    //    🔴 Both tiers are needed: the global pack carries the chrome glyphs, the ToolMenu pack every tool/band/parameter glyph
    //       in TWO inks (live and gated), because a rasterized SVG cannot be recoloured after upload.
    SvgIconRegistry Icons;
    if (!InitializeSvgIconRegistry(Icons, Host))
    {
        fprintf(stderr, "[toolcard-validation] SVG icon registry failed to start\n");
    }
    else
    {
        const bool GlobalOk   = RegisterGlobalIconPack(Icons);
        const bool ToolMenuOk = RegisterToolMenuIconPack(Icons);
        if (!GlobalOk || !ToolMenuOk)
        {
            fprintf(stderr, "[toolcard-validation] icon pack registration incomplete (global=%d toolmenu=%d)\n",
                    (int)GlobalOk, (int)ToolMenuOk);
        }
    }

    // -- The caller-owned card state, seeded to the prototype's opening pose ----------------------------------------------
    ModellingToolState State;
    InitializeModellingToolSample(State);
    ...
    while (!QueryWindowCloseRequested(Window))
    {
        ...
        // 📝 One docked-full window hosting the panel: selection authoring on the left, the live card on the right.
        ...
        if (ImGui::Begin("Modelling Tool Card", nullptr, HostFlags))
        {
            ConstructModellingToolPanel(Theme, State, &Icons);
        }
        ImGui::End();
        ImGui::PopStyleVar();
