@echo off
REM ============================================================================
REM  InstanceTreeValidation\Build.bat - build + run InstanceTreeValidation.exe:
REM  the exit gate for the TOP-level (TLAS) build's last two dispatches. It runs
REM  the WHOLE four-dispatch chain on a REAL DEVICE - reduce, Morton, radix sort,
REM  tree build, refit - so it links Graphics.lib (all four submissions + the
REM  Vulkan host) and EngineContext.lib, plus vulkan-1.lib.
REM
...
set "NAME=InstanceTreeValidation"
