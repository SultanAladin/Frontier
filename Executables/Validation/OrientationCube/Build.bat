@echo off
REM ============================================================================
REM  OrientationCube\Build.bat - build OrientationCube.exe, a STANDALONE
REM  validation window that drives one live perspective ViewportPanel so the
REM  reference grid + orientation-cube overlay can be eyeballed + orbited.
REM
REM  Win32 + Direct3D 11 host. It links ONLY EngineContext.lib (the pillar that
REM  carries the viewport panel + grid + cube overlay + theme, all drawn on the
REM  ImGui draw list) + the vendored ImGui core it compiles itself (core +
REM  Win32/DX11 backends) + system D3D11. No Graphics / Platform / Vulkan - the
REM  viewport records on the ImGui draw list, not GroundGridPass. Validation
REM  hosts keep the proven D3D11 backend; the editors run on native Vulkan.
REM
...
REM  EngineContext carries the viewport panel + grid + cube overlay + theme.
...
for %%U in (OrientationCubeHost) do (
    echo [compile] %%U.cpp
