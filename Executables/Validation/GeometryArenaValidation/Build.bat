@echo off
REM ============================================================================
REM  GeometryArenaValidation\Build.bat - build + run GeometryArenaValidation.exe:
REM  the exit gate for the bottom-level tree arena. Like the bounds gate this one
REM  needs a REAL DEVICE - it uploads node blobs and reads them back - so it links
REM  Graphics.lib (the arena submission + the CPU tree builder + the Vulkan host)
REM  and EngineContext.lib (the diagnostics the host reports through), plus
REM  vulkan-1.lib.
REM
...
REM
REM  NO SHADER STEP. The arena owns buffers and dispatches nothing, so there is
REM  no .comp to keep current and ShaderPlan is not invoked.
REM
...
set "NAME=GeometryArenaValidation"
