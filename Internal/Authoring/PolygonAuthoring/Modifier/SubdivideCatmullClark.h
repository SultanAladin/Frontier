...
============================================================================================================================================*/
// ðŸ§© One level of CATMULL-CLARK subdivision on an indexed-face polygon â€” the smooth quad scheme (Catmull & Clark 1978). Unlike
...
// ðŸ“ Robustness beyond a plain implementation (the "more than Blender" target): (1) OPEN BOUNDARIES follow the boundary curve â€”
//    a boundary edge point is its midpoint and a boundary vertex uses the (Prev + 6Â·P + Next)/8 crease rule, so open surfaces do
//    not shrink inward. (2) SEMI-SHARP CREASES â€” a per-edge continuous EdgeSharpness in [0,1] linearly blends each edge point and
...
#pragma once
#ifndef FRONTIER_AUTHORING_POLYGONAUTHORING_MODIFIER_SUBDIVIDECATMULLCLARK_H
#define FRONTIER_AUTHORING_POLYGONAUTHORING_MODIFIER_SUBDIVIDECATMULLCLARK_H
...
// EdgeSharpness maps a canonical undirected edge key (lower vertex index in the high 32 bits, higher in the low 32 â€” matching
...
// a hard border). Outputs are the refined polygon in the same flat format â€” original vertices first (repositioned), then one
...
#endif   // FRONTIER_AUTHORING_POLYGONAUTHORING_MODIFIER_SUBDIVIDECATMULLCLARK_H
