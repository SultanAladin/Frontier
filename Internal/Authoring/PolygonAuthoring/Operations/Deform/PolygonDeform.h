/*============================================================================================================================================
                                                              POLYGONDEFORM.H
============================================================================================================================================*/
// ðŸ§© Three Blender-standard vertex-deform kernels for the authoring PolygonCluster â€” Smooth, Shrink/Fatten, and Randomize. Unlike
...
//    stay valid (the caller need not rebuild adjacency for topology â€” only re-stream the moved positions to the device). Smooth is an
...
#pragma once
#ifndef FRONTIER_AUTHORING_POLYGONAUTHORING_OPERATIONS_DEFORM_POLYGONDEFORM_H
#define FRONTIER_AUTHORING_POLYGONAUTHORING_OPERATIONS_DEFORM_POLYGONDEFORM_H
...
struct PolygonCluster;   // ðŸ“ forward-declared â€” the kernels mutate its vertex positions only; the .cpp pulls the full header.
struct AdjacencyIndex;   // ðŸ“ forward-declared â€” the kernels read one-ring adjacency / face normals over the BASE topology only.

//------------------------------------------------------------------------------------------------------------------------
//                                                          ENUMERATIONS
//------------------------------------------------------------------------------------------------------------------------

// ðŸ“ The randomize axis, taken from the toolbar's Axis choice. `.Category` suffix per the naming skill (replaces `Kind`). Uniform
...
// ðŸ“ One affected vertex and the axes an interactive deform slides it along, captured at arm time. NormalDirection is the unit
...
// ðŸ“ The snapshot an interactive deform (Shrink / Randomize) captures at arm time: one sample per affected vertex. The viewport
...
// and its FULLY relaxed position (SmoothedTarget) â€” the Laplacian result of Iterations passes at Factor 1.0 over the same one-ring
...
// exactly to the origin. Strength is clamped to [0, 1] â€” 0 leaves the vertices at rest, 1 is the fully relaxed target. Returns false
...
// splitmix64(Seed ^ vertexIndex) â€” deterministic, so the same Seed re-rolls the identical pattern and no runtime RNG is needed.
...
// RandomDirection's magnitude bookkeeping â€” so every vertex jitters by a different signed magnitude, deterministically from the
...
#endif   // FRONTIER_AUTHORING_POLYGONAUTHORING_OPERATIONS_DEFORM_POLYGONDEFORM_H
