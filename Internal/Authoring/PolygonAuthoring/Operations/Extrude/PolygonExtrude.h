/*============================================================================================================================================
                                                              POLYGONEXTRUDE.H
============================================================================================================================================*/
// ðŸ§© The Blender-standard face-extrude kernel for the authoring PolygonCluster: it clones a selected face set into a new cap,
...
#pragma once
#ifndef FRONTIER_AUTHORING_POLYGONAUTHORING_OPERATIONS_EXTRUDE_POLYGONEXTRUDE_H
#define FRONTIER_AUTHORING_POLYGONAUTHORING_OPERATIONS_EXTRUDE_POLYGONEXTRUDE_H
...
struct AdjacencyIndex;   // ðŸ“ forward-declared â€” the kernel reads loops / normals / edge incidence over the BASE topology only.

//------------------------------------------------------------------------------------------------------------------------
//                                                          ENUMERATIONS
//------------------------------------------------------------------------------------------------------------------------

// ðŸ“ The extrude variant, taken from the toolbar's Type dropdown (Manifold is booleans â€” out of scope, not represented here).
...
// ðŸ“ One appended cap vertex and the unit axis it slides along while the caller drives the interactive offset. OriginPosition
//    is the zero-offset position (identical to the base vertex the clone was taken from), so ExtrudeOffsetAdvance re-derives the
//    live position idempotently as OriginPosition + Direction * Distance â€” the same origin-snapshot rule the drag gizmo uses.
...
// ðŸ“ The whole outcome of a zero-offset extrude: what topology was appended and how to drive the live offset. CapFaces /
...
#endif   // FRONTIER_AUTHORING_POLYGONAUTHORING_OPERATIONS_EXTRUDE_POLYGONEXTRUDE_H
