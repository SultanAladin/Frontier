...
============================================================================================================================================*/
// 🧩 The CPU single-click picker, cluster-native path: cast one object-space Ray against a DisplayPolygons' triangles, keep the
//    nearest hit, and resolve it — through the TriangleOrigin provenance + the AdjacencyIndex — back to a source face / nearest
...
//    component to select. No half-edge (TopologyStructure) is needed — the non-edit path resolves entirely off the cluster.
// 📝 Classification is barycentric (resolution-independent, enough for the authored tri/quad/N-gon cases): the largest corner
...
#pragma once
#ifndef FRONTIER_AUTHORING_MODELING_PICKING_RAYPICKINTERSECTION_H
#define FRONTIER_AUTHORING_MODELING_PICKING_RAYPICKINTERSECTION_H
...
struct AdjacencyIndex;   // 📝 forward-declared; resolves the loop-edge test for classification

//------------------------------------------------------------------------------------------------------------------------
//                                                            CONSTANTS
//------------------------------------------------------------------------------------------------------------------------

// 📝 Sentinels for "no component resolved". The face ordinal / vertex index reuse InvalidCornerReference (0xFFFFFFFF); the edge
...
// 📝 The outcome of one component pick. HitEnabled is false for a ray that missed every triangle (all other fields are then
...
//    nearest real loop edge's packed key — InvalidEdgeKey on a fan diagonal.
struct PickOutcome
{
...
#endif   // FRONTIER_AUTHORING_MODELING_PICKING_RAYPICKINTERSECTION_H
