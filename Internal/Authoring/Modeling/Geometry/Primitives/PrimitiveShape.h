/*============================================================================================================================================
                                                              PRIMITIVESHAPE.H
============================================================================================================================================*/
// 🧩 Procedural primitive generator: twenty parametrized base shapes, each constructing the engine's single authoring container
//    (PolygonCluster — VertexField positions in double cm + a flat FaceVertexIndices / FaceVertexCounts face list that keeps
...
//    container + the Math library — no Vulkan, no Interface. Round shapes share the cap-style + loop-count parameter pattern
//    (see CapCategory).

#pragma once
#ifndef FRONTIER_AUTHORING_MODELING_GEOMETRY_PRIMITIVES_PRIMITIVESHAPE_H
#define FRONTIER_AUTHORING_MODELING_GEOMETRY_PRIMITIVES_PRIMITIVESHAPE_H
...
// 📝 The twenty base primitives. PrimitiveShapeCount stays last so a caller can iterate the whole set (the force-test
...
// 📝 How a round primitive closes an open circular end. Shared by Cylinder / Cone / Tube / Disc so the cylinder example
...
// 📝 One parameter block per shape. Defaults describe a ~200 cm well-formed unit so ConstructPrimitiveDefault yields a
...
// 📝 Per-shape constructors. Each returns a fully-formed PolygonCluster (positions in authoring cm, CCW-outward winding,
//    quads where a face is naturally four-sided) with its Descriptor already refreshed. A degenerate parameter (a count
//    below the minimum a closed shell needs) is clamped up, never rejected — a primitive always yields a valid body.
...
// Human-readable name for a category (stable, ASCII, no spaces) — used for object titles + logging. Returns "unknown"
// for an out-of-range category.
const char* ResolvePrimitiveShapeName(PrimitiveShapeCategory Category);

} // namespace Frontier

#endif   // FRONTIER_AUTHORING_MODELING_GEOMETRY_PRIMITIVES_PRIMITIVESHAPE_H
