...
============================================================================================================================================*/
// 🧩 The display mirror's entry point: ConstructDisplayPolygons derives the throwaway GPU triangle view (plus its provenance
//    map) from the authoring PolygonCluster — the single source of truth on the non-edit path (§2.1). The cluster is
...
//    to recover it (§3.3). Double->float conversion happens only here, at this presentation boundary.
// 📝 A separate half-edge (TopologyStructure) overload is a later edit-mode concern; this phase renders + picks straight off
//    the cluster, so only the cluster overload exists here.

#pragma once
#ifndef FRONTIER_AUTHORING_MODELING_GEOMETRY_DISPLAY_DISPLAYPOLYGONASSEMBLY_H
#define FRONTIER_AUTHORING_MODELING_GEOMETRY_DISPLAY_DISPLAYPOLYGONASSEMBLY_H
...
// ConstructRenderVertexStream §3.1). Each face is fan-triangulated (v0, vi, vi+1); its corners are expanded into interleaved
// float RenderVertices (no dedup — one render vertex per corner reference, so seams stay split), and one TriangleOrigin is
...
// resolved from FaceCornerTexture, then the per-vertex VertexField UV, then zero — identical to ConstructRenderVertexStream.
//
...
#endif   // FRONTIER_AUTHORING_MODELING_GEOMETRY_DISPLAY_DISPLAYPOLYGONASSEMBLY_H
