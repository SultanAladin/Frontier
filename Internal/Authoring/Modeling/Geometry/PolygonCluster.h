/*============================================================================================================================================
                                                               POLYGONCLUSTER.H
============================================================================================================================================*/
// 🧩 The CPU authoring polygon container and its derived GPU presentation. The authoring side (VertexField + a face list that
...
//    re-runnable function and is never read back to recover authoring topology (anti-fragility §3.3). Float conversion happens
//    only here, at the presentation boundary, exactly as the Math library and Camera intend.

#pragma once
#ifndef FRONTIER_AUTHORING_MODELING_GEOMETRY_POLYGONCLUSTER_H
#define FRONTIER_AUTHORING_MODELING_GEOMETRY_POLYGONCLUSTER_H
...
// 📝 Authoring polygon. Faces are stored as a flat index stream plus a per-face corner count, so a triangle (3), a quad (4),
...
//
// 📝 FaceCornerTexture is the persisted home of PER-CORNER UVs: one Vector2d per entry of FaceVertexIndices (same length, same
//    order), so a seam vertex shared by two faces holds two distinct corner UVs — the truth painting / UV editing later read.
...
// 📝 One interleaved GPU vertex. Float, tightly packed, and laid out exactly as a future Vulkan vertex-input binding will
//    declare it (position @0, normal @12, texcoord @24; stride 32). This is the engine's single polygon presentation contract
//    — CAD tessellation and primitive generators must all emit THIS type so the renderer never branches on producer (§6).
struct RenderVertex
{
...
// 📝 The derived, triangulated GPU stream: a deduplication-free expansion for now (one render vertex per authoring corner
...
// 📝 CornerColour + CornerMaterialIndex are BAKER-ONLY parallel arrays (one entry per Vertices entry): the renderer's
//    stride-32 vertex-input contract never reads them, so they carry authoring colour / material identity into the bake
//    pipeline without widening RenderVertex. Both stay empty (the default) for producers that emit no colour / material —
//    ConstructTriangleRayVolume then substitutes white / material 0 — so all non-bake callers are unaffected.
...
// Derive the GPU presentation for the WHOLE polygon in one aggregate pass (anti-fragility §3.1 — no per-face public entry).
...
// Release authoring attributes, face arrays, and reset the descriptor to its default (identity preserved is NOT assumed —
