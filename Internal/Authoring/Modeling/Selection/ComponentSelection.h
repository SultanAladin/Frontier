...
============================================================================================================================================*/
// 🧩 The component-selection state + operations for the non-edit (cluster) path — the engine-named port of the reference
...
// 📝 This reads only an AdjacencyIndex (the halfedge.js TOPO shape) — no half-edge, no cluster mutation — exactly as the
...
#pragma once
#ifndef FRONTIER_AUTHORING_MODELING_SELECTION_COMPONENTSELECTION_H
#define FRONTIER_AUTHORING_MODELING_SELECTION_COMPONENTSELECTION_H
...
struct AdjacencyIndex;   // 📝 forward-declared; every operation reads its loops / adjacency / edge incidence
struct PolygonCluster;   // 📝 forward-declared; the loop walk's straightness fallback reads its vertex positions

//------------------------------------------------------------------------------------------------------------------------
//                                                            STRUCTS
//------------------------------------------------------------------------------------------------------------------------

// 📝 The single anchor for a two-pick path/loop gesture (the prototype's SEL.lastPick). Enabled is false until the first
...
// 📝 The whole selection state — three parallel active sets (only the active mode's set is authoritative for a gesture, the
...
// 📝 The ghost set a modifier-held click would grab, computed on hover without mutating the live selection (the prototype's
...
// Face / Island -> Faces); the hit sets come straight from ResolveRegionSelection. LastPick is untouched — a region has no
...
// Select the whole edge loop grown from SeedEdge (the port of selLoopFromEdge → computeLoopEdges): the vertex-centric
...
// Select the face loop / ring grown from SeedFace (the port of selRingFromFace → computeFaceBand): from a quad seed, the
...
// face-pool gesture). Updates LastPick. Mirrors ResolveFaceLoopSelection — the highlight is automatic in both surfaces.
...
// Grow the active-mode selection by one ring — add every component adjacent to the current selection (the port of selGrow, Blender
...
// Shrink the active-mode selection by one ring — remove every boundary component that touches an unselected adjacent (the port of
...
// ghost (the port of buildLoopPreview → loopEdgeSet / computeFaceBand). In Edge mode HoverEdge seeds an edge loop into
// Preview.Edges; in Face mode HoverFace seeds a face band into Preview.Faces; other modes leave Preview empty. Clears Preview
// first; HoverValid false means no hover → empty preview. The unused hover argument is ignored per mode.
...
#endif   // FRONTIER_AUTHORING_MODELING_SELECTION_COMPONENTSELECTION_H
