...
//                                                              DraughtLoft.cpp
//==========================================================================================================================================
// 🧩 Lofting kernel — interpolate a 3D display surface / solid through 2+ section profiles. See DraughtLoft.h for the orchestration
...
namespace
{
    ...
    uint32_t PushVertex(DraughtLoftBody& Body, float X, float Y, float Z)
    {
    ...
    void AccumulateTriangleNormal(DraughtLoftBody& Body, uint32_t A, uint32_t B, uint32_t C)
    {
    ...
    void AppendTriangle(DraughtLoftBody& Body, uint32_t A, uint32_t B, uint32_t C)
    {
    ...
    void NormalizeBodyNormals(DraughtLoftBody& Body)
    {
    ...
    void AppendCap(DraughtLoftBody& Body, const std::vector<ImVec2>& Ring, float Elevation)
    {
        if (Ring.size() < 3)
            return;
...
namespace
{
    ...
    bool ResolveLoftSections(DraughtShapeStore&       Store,
...
namespace
{
    ...
    void InterpolateLoftSurface(DraughtLoftBody&                Body,
    ...
    void GenerateLoftGeometry(DraughtLoftBody&                Body,
                              const std::vector<LoftSection>& Sections,
                              const LoftSpecification&        Spec)
    {
...
LoftOutcome AppendLoftResult(DraughtShapeStore& Store, const LoftSpecification& Spec)
{
    ...
    AppendDraughtEdit(Store, 0, LogLabel, "spline");
...
void ReflowLoftBodies(DraughtShapeStore& Store)
{
    ...
    {
        DraughtLoftBody& Body = Store.LoftBodies[Index];
...
DraughtLoftBody* ResolveLoftBody(DraughtShapeStore& Store, uint32_t Identifier)
{
    if (Identifier == 0)
        return nullptr;
    for (DraughtLoftBody& Body : Store.LoftBodies)
