...
namespace
{
    constexpr float TwoPi     = 6.28318530717958647692f;   // [rad] - a full turn (matches DraughtShapeStore's constant)
    ...
    bool ResolveCornerTriple(const DraughtShape& Shape,
                             int                 Index,
                             ImVec2&             Previous,
                             ImVec2&             Corner,
                             ImVec2&             Next)
    ...
    //    DraughtCornerFillet (CornerIndex + Magnitude + ChamferEnabled) on the shape — leaving Points untouched so the corner stays ONE real
    ...
    bool ApplyCornerEdit(DraughtShapeStore&    Store,
                         uint32_t              Identifier,
                         int                   Index,
                         float                 Magnitude,
                         DraughtCornerCategory Category)
    {
        ...
        if (Magnitude > 0.0f)
        {
            const DraughtCornerSolution Solution = SolveCornerEdit(*Shape, Index, Magnitude, Category);
            if (!Solution.Resolved)
                return false;
        }

        const bool ChamferEnabled = (Category == DraughtCornerCategory::Chamfer);
        ...
        DraughtCornerFillet* Existing = nullptr;
        for (DraughtCornerFillet& Candidate : Shape->CornerFillets)
        ...
        AppendDraughtEditDetailed(Store, Identifier, LogLabel, ChamferEnabled ? "scissors" : "git-commit-horizontal", LogDetail);
...
DraughtCornerSolution SolveCornerEdit(const DraughtShape&    Shape,
                                      ...
                                      DraughtCornerCategory  Category)
{
    DraughtCornerSolution Solution;
    if (Magnitude <= 0.0f)
        return Solution;
    ...
    Solution.SafeLimit = (Category == DraughtCornerCategory::Fillet) ? RadiusCeiling : ShortestLeg;
    ...
    if (Category == DraughtCornerCategory::Fillet)
    {
...
float ResolveCornerSafeLimit(const DraughtShape& Shape, int Index)
{
    ...
    const DraughtCornerSolution Probe = SolveCornerEdit(Shape, Index, CornerEps * 2.0f, DraughtCornerCategory::Fillet);
...
{
    return ApplyCornerEdit(Store, Identifier, Index, Magnitude, DraughtCornerCategory::Fillet);
}

bool ChamferShapeCorner(DraughtShapeStore& Store, uint32_t Identifier, int Index, float Magnitude)
{
    return ApplyCornerEdit(Store, Identifier, Index, Magnitude, DraughtCornerCategory::Chamfer);
}
