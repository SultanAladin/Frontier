...
//    analytic — no pixels stored. Under the Authoring pillar (Authoring/Modeling/Draughting) as the sketch layer's kernel.
...
namespace
{
    ...
    const char* ShapeCategoryLabel(DraughtShapeCategory Category)
    {
        switch (Category)
        {
            case DraughtShapeCategory::Line:      return "Line";
            case DraughtShapeCategory::Polyline:  return "Polyline";
            case DraughtShapeCategory::Arc:       return "Arc";
            case DraughtShapeCategory::Bezier:    return "Bezier";
            case DraughtShapeCategory::BSpline:   return "B-Spline";
            case DraughtShapeCategory::Nurbs:     return "NURBS";
            case DraughtShapeCategory::Spline:    return "Spline";
            case DraughtShapeCategory::Conic:     return "Conic";
            case DraughtShapeCategory::Rectangle: return "Rectangle";
            case DraughtShapeCategory::Circle:    return "Circle";
            case DraughtShapeCategory::Ellipse:   return "Ellipse";
            case DraughtShapeCategory::Polygon:   return "Polygon";
            case DraughtShapeCategory::Slot:      return "Slot";
    ...
    const char* ShapeCategoryGlyph(DraughtShapeCategory Category)
    {
        switch (Category)
        {
            case DraughtShapeCategory::Line:      return "minus";
            case DraughtShapeCategory::Polyline:  return "spline";
            case DraughtShapeCategory::Arc:       return "git-commit-horizontal";
            case DraughtShapeCategory::Bezier:    return "pen-tool";
            case DraughtShapeCategory::BSpline:   return "spline";
            case DraughtShapeCategory::Nurbs:     return "spline";
            case DraughtShapeCategory::Spline:    return "spline";
            case DraughtShapeCategory::Conic:     return "git-commit-horizontal";
            case DraughtShapeCategory::Rectangle: return "square";
            case DraughtShapeCategory::Circle:    return "circle";
            case DraughtShapeCategory::Ellipse:   return "egg";
            case DraughtShapeCategory::Polygon:   return "hexagon";
            case DraughtShapeCategory::Slot:      return "rectangle-horizontal";
            default:                              return "dot";
        }
    }
    ...
    ImVec2 EvaluateEllipsePosition(const DraughtShape& Shape, float T)
    {
    ...
    //    a zero leg, a non-positive magnitude). Self-contained analytic geometry (mirrors DraughtFillet's SolveCornerEdit) so the flatten
...
{
    for (DraughtShapeStore& Entry : Registry.Stores)
        if (Entry.OwnerDocument == OwnerDocument)
            return Entry;
    DraughtShapeStore Fresh;
...
// 📝 The active draughting store the runtime publishes each frame so the separate outliner / Properties / History boxes read + edit
//    the same shapes without threading a pointer through the dock-host chain. Null outside a live draughting view. Mirrors the Scene
//    outliner's RegisterSceneOutlinerSource bridge exactly.
namespace
{
    DraughtShapeStore* PublishedShapeSource = nullptr;
    DraughtSceneView   PublishedSceneView;     // [-] - reset to a not-ready default each time the runtime clears the bridge
    ...
    std::vector<DraughtShapeBody> PublishedShapeBodies;        // [-] - tessellated closed-shape / solid bodies the GPU scene pass draws
}
...
int ResolveDraughtDefiningCount(DraughtShapeCategory Category)
{
    switch (Category)
    {
        case DraughtShapeCategory::Line:      return 2;
        case DraughtShapeCategory::Arc:       return 3;
        case DraughtShapeCategory::Conic:     return 3;
        case DraughtShapeCategory::Rectangle: return 2;
        case DraughtShapeCategory::Circle:    return 2;
        case DraughtShapeCategory::Ellipse:   return 3;
        case DraughtShapeCategory::Polygon:   return 2;
        case DraughtShapeCategory::Slot:      return 3;
                                   ...
                                   int                        DegreeOverride)
{
    ...
    switch (Category)
    {
        ...
        case DraughtShapeCategory::Circle:
        {
        ...
        case DraughtShapeCategory::Ellipse:
        {
        ...
        case DraughtShapeCategory::Polygon:
        {
        ...
        case DraughtShapeCategory::Slot:
        {
        ...
        case DraughtShapeCategory::Conic:
        {
...
void EvaluateShapePolyline(const DraughtShape& Shape, std::vector<ImVec2>& Polyline, int SampleBudget)
{
    Polyline.clear();
    ...
    if (!Shape.CornerFillets.empty() && Polyline.size() == Shape.Points.size() && Shape.Points.size() >= 3)
    {
        ...
        for (int Walk = 0; Walk < Count; ++Walk)
        {
            ...
            const DraughtCornerFillet* Edit = nullptr;
            for (const DraughtCornerFillet& Candidate : Shape.CornerFillets)
...
const std::vector<ImVec2>& RetrieveCachedOutline(DraughtShape& Shape, int SampleBudget)
{
...
int ResolveAdaptiveSampleBudget(const DraughtShape& Shape, float PixelsPerMm)
{
    ...
    if (Shape.Category == DraughtShapeCategory::Polygon)
        return 0;
...
uint32_t AppendDraughtShape(DraughtShapeStore&         Store,
                            ...
                            int                        DegreeOverride)
{
    const int Required = ResolveDraughtDefiningCount(Category);
    ...
    DraughtShape Fresh = ConstructDraughtShape(Category, DefiningPoints, SideCountOverride, RhoOverride, DegreeOverride);
    ...
    char LogDetail[64];
    std::snprintf(LogDetail, sizeof(LogDetail), "Points=%d;Length=%.2f mm", (int)Fresh.Points.size(), EvaluateDraughtShapeLength(Fresh));
    AppendDraughtEditDetailed(Store, Fresh.Identifier, LogLabel, ShapeCategoryGlyph(Category), LogDetail);
...
DraughtShape* ResolveDraughtShape(DraughtShapeStore& Store, uint32_t Identifier)
{
    if (Identifier == 0)
        return nullptr;
    for (DraughtShape& Entry : Store.Shapes)
...
{
    DraughtInscription Fresh;
    Fresh.Identifier = Store.NextInscriptionIdentifier++;
    Fresh.Anchor     = AnchorMm;
    Fresh.TintIndex  = Fresh.Identifier;   // a rolling cue; the outliner maps it into its tint ladder (mirrors AppendDraughtShape)
    ...
    AppendDraughtEdit(Store, 0, LogLabel, "type");
...
DraughtInscription* ResolveInscription(DraughtShapeStore& Store, uint32_t Identifier)
{
    if (Identifier == 0)
        return nullptr;
    for (DraughtInscription& Entry : Store.Inscriptions)
        if (Entry.Identifier == Identifier)
            return &Entry;
    return nullptr;
}
...
void DetachInscription(DraughtShapeStore& Store, uint32_t Identifier)
{
    ...
    AppendDraughtEdit(Store, 0, LogLabel, "inscribe");
}
...
uint32_t AppendDatum(DraughtShapeStore& Store, ImVec2 AnchorMm)
{
    ...
    Fresh.Anchor      = AnchorMm;
    Fresh.AxisDisplay = DraughtDatumAxis::Cross;   // default: a full cross so a fresh datum reads as a reference frame immediately
    ...
    AppendDraughtEdit(Store, 0, LogLabel, "crosshair");
...
DraughtDatum* ResolveDatum(DraughtShapeStore& Store, uint32_t Identifier)
{
    if (Identifier == 0)
        return nullptr;
    for (DraughtDatum& Entry : Store.Datums)
        if (Entry.Identifier == Identifier)
            return &Entry;
    return nullptr;
}
...
uint32_t PickDatum(DraughtShapeStore& Store, ImVec2 WorldPoint, float ToleranceMm)
{
    ...
    for (const DraughtDatum& Entry : Store.Datums)
    {
...
float EvaluateDistanceToShape(DraughtShape& Shape, ImVec2 WorldPoint, int SampleBudget)
{
...
void EvaluateFilledPolygon(const DraughtShape& Shape, std::vector<ImVec2>& OutPolyline, int SampleBudget)
{
    OutPolyline.clear();
                                          ...
                                          unsigned           Mask)
{
    ...
    auto Consider = [&](ImVec2 Candidate, DraughtSnapCategory Category)
    {
        ...
        const bool  Endpoint = Category == DraughtSnapCategory::Endpoint;
        const float Ceiling  = Endpoint ? NearestSquared : SoftSquared;
        if (DistanceSquared > Ceiling)
            return;
        if (Best.Resolved && Best.Category == DraughtSnapCategory::Endpoint && !Endpoint)
    ...
    const bool EndpointEnabled = (Mask & (1u << (unsigned)DraughtSnapCategory::Endpoint))   != 0;
    const bool MidpointEnabled = (Mask & (1u << (unsigned)DraughtSnapCategory::Midpoint))   != 0;
    const bool CenterEnabled   = (Mask & (1u << (unsigned)DraughtSnapCategory::Center))     != 0;
    const bool AlongEnabled    = (Mask & (1u << (unsigned)DraughtSnapCategory::AlongCurve)) != 0;
    ...
    if (EndpointEnabled && Store.DrawingEnabled)
        for (const ImVec2& Pending : Store.PendingPoints)
            Consider(Pending, DraughtSnapCategory::Endpoint);
    ...
    for (DraughtShape& Shape : Store.Shapes)
    {
        ...
        if (EndpointEnabled)
        {
            for (const ImVec2& Point : Shape.Points)
                Consider(Point, DraughtSnapCategory::Endpoint);
            ...
            {
                Consider(Shape.CachedOutline.front(), DraughtSnapCategory::Endpoint);
                Consider(Shape.CachedOutline.back(),  DraughtSnapCategory::Endpoint);
            }
        }
        ...
        if (CenterEnabled)
            switch (Shape.Category)
            {
                ...
                case DraughtShapeCategory::Polygon:
                case DraughtShapeCategory::Slot:
                    Consider(Shape.Centre, DraughtSnapCategory::Center);
                    break;
                default:
                    break;
            }
        ...
        if (MidpointEnabled && Shape.Category == DraughtShapeCategory::Arc && Shape.Radius > 1e-4f)
        {
            ...
            for (size_t Index = 0; Index + 1 < Count; ++Index)
            {
                ...
                if (AlongEnabled)
                    Consider(NearestOnSegment(WorldCursor, Polyline[Index], Polyline[Index + 1]),
                             DraughtSnapCategory::AlongCurve);
            }
            if (Shape.ClosedEnabled && Count >= 3)
            {
                if (MidpointEnabled)
...
float ResolvePrimaryDimension(const DraughtShape& Shape, const char** PrimaryLabel)
{
    switch (Shape.Category)
    {
        ...
        default:
            ...
            return EvaluateDraughtShapeLength(Shape);
    }
}
...
void EnforcePrimaryDimension(DraughtShape& Shape, float TargetMillimetres)
{
    ...
    switch (Shape.Category)
    {
        ...
        default:
        {
            ...
            const float Current = EvaluateDraughtShapeLength(Shape);
...
{
    if (Shape.Category != DraughtShapeCategory::Ellipse || MajorMillimetres <= 0.0f || MinorMillimetres <= 0.0f)
        return;
...
void ResolveRectangleDimensions(const DraughtShape& Shape, float& Width, float& Height)
{
    ...
    Width  = 0.0f;
    Height = 0.0f;
    if (Shape.Category != DraughtShapeCategory::Rectangle || Shape.Points.empty())
        return;
...
{
    if (Shape.Category != DraughtShapeCategory::Rectangle || WidthMillimetres <= 0.0f || HeightMillimetres <= 0.0f ||
        Shape.Points.empty())
        return;

    // Rebuild the four corners centred on the CURRENT box centre at the requested span, so only the size changes — position holds.
    //    Re-solve through ConstructDraughtShape (two opposite corners) so the closed loop + cache invalidation match every draw path.
    float MinimumX = Shape.Points[0].x, MaximumX = Shape.Points[0].x;
    float MinimumY = Shape.Points[0].y, MaximumY = Shape.Points[0].y;
    for (const ImVec2& Corner : Shape.Points)
    {
    ...
    DraughtShape Solved     = ConstructDraughtShape(DraughtShapeCategory::Rectangle, Corners);
...
{
    if (Shape.Category != DraughtShapeCategory::Ellipse && Shape.Category != DraughtShapeCategory::Polygon)
        return;
    ...
    if (Shape.Points.size() >= 2)
    {
        const float Reach = Shape.Category == DraughtShapeCategory::Ellipse ? Shape.MajorAxis : Shape.Radius;
...
{
    if (Shape.Category != DraughtShapeCategory::Conic)
        return;
...
{
    if (Shape.Category != DraughtShapeCategory::BSpline && Shape.Category != DraughtShapeCategory::Nurbs)
        return;
...
// 📝 Live-mirror reflow — defined in DraughtTransform.cpp (which owns the reflection math). Forward-declared here (DraughtShapeStore.h must
//    not include DraughtTransform.h — the dependency runs the other way) so every committed edit refreshes the driven mirror children BEFORE
//    the history snapshot, keeping parent + copies coherent across undo / redo.
void ReflowMirrorChildren(DraughtShapeStore& Store);

// 📝 Live-array reflow — likewise defined in DraughtTransform.cpp. Forward-declared here so a committed edit refreshes the driven ARRAY copies
...
// 📝 Live-loft reflow — defined in DraughtLoft.cpp. Forward-declared here so a committed edit re-solves every lofted body from its live source
//    sections (after the mirror / array copies, before the snapshot), keeping each DraughtLoftBody coherent with its profiles across undo/redo.
                               ...
                               const char*        Detail)
{
    ...
    CaptureDraughtRevision(Store, Entry.Revision);
...
{
    AppendDraughtEditDetailed(Store, ShapeId, Label, Glyph, nullptr);
}
...
void RestoreDraughtRevisionAt(DraughtShapeStore& Store, int Step)
{
    ...
    {
        const DraughtRevision& Revision = Store.EditLog[Step - 1].Revision;
    ...
    if (Store.Selected != 0 && !ResolveDraughtShape(Store, Store.Selected))
        Store.Selected = 0;
    ...
    if (Store.SelectedLoft != 0)
    {
        ...
        for (const DraughtLoftBody& Body : Store.LoftBodies)
    ...
    {
        DraughtShape* DatumOwner = ResolveDraughtShape(Store, Store.DatumSelected);
...
bool UndoDraughtEdit(DraughtShapeStore& Store)
{
    if (Store.LogCursor <= 0)
        return false;
    RestoreDraughtRevisionAt(Store, Store.LogCursor - 1);
    return true;
}
...
uint32_t AttachDraughtFolder(DraughtShapeStore& Store, const char* Title)
{
    ...
    AppendDraughtEdit(Store, 0, LogLabel, "folder-plus");
...
{
    DraughtShape* Source = ResolveDraughtShape(Store, Identifier);
    if (!Source)
        return 0;
    ...
    const DraughtShapeCategory Category = Source->Category;
    const int   SideCount = Source->SideCount;
    const float Rho       = Source->Rho;
    const int   Degree    = Source->Degree;

    DraughtShape Fresh = ConstructDraughtShape(Category, Points, SideCount, Rho, Degree);
    Fresh.Identifier    = Store.NextIdentifier++;
    ...
    AppendDraughtEdit(Store, Fresh.Identifier, LogLabel, "copy");
...
void DetachDraughtShape(DraughtShapeStore& Store, uint32_t Identifier)
{
    ...
    {
        const DraughtConstraint& Constraint = Store.Constraints[Index];
    ...
    {
        const DraughtDimension& Dimension = Store.Dimensions[Index];
    ...
    AppendDraughtEdit(Store, 0, LogLabel, "trash-2");
}
...
namespace
{
    ...
    void BuildWorkingPolyline(DraughtShape& Shape, std::vector<ImVec2>& Out, bool& Closed)
    {
    ...
    void CollectIntersectionParameters(DraughtShapeStore&         Store,
                                       ...
                                       std::vector<float>&        Breaks)
    {
        ...
        for (DraughtShape& Other : Store.Shapes)
        {
    ...
    bool StraightRunFamily(DraughtShapeCategory Category)
    {
        return Category == DraughtShapeCategory::Line ||
               Category == DraughtShapeCategory::Polyline ||
               Category == DraughtShapeCategory::Rectangle ||
               Category == DraughtShapeCategory::Profile;
    }
    ...
    void CollectAnalyticCornerBreaks(const DraughtShape&        Shape,
                                     const std::vector<ImVec2>& W,
                                     bool                       Closed,
                                     const std::vector<float>&  CumulativeLength,
                                     std::vector<float>&        Breaks)
    {
        ...
        for (int Corner = 0; Corner < CornerCount; ++Corner)
        {
            ...
            if (!Endpoint)
                for (const DraughtCornerFillet& Edit : Shape.CornerFillets)
    ...
    void ReplaceShapeWithPolyline(DraughtShapeStore&         Store,
                                  uint32_t                   Identifier,
                                  const std::vector<ImVec2>& Run)
    {
        DraughtShape* Live = ResolveDraughtShape(Store, Identifier);
        if (Live == nullptr)
            return;
        ...
        *Live = ConstructDraughtShape(DraughtShapeCategory::Polyline, Run);
    ...
    uint32_t AppendArcPiece(DraughtShapeStore& Store,
                            ImVec2             Centre,
                            float              Radius,
                            float              AngleStart,
                            float              AngleEnd,
                            ...
                            uint32_t           Folder)
    {
        ...
        const uint32_t Identifier = AppendDraughtShape(Store, DraughtShapeCategory::Arc, { Start, End, Through });
        if (Identifier != 0)
        {
            if (DraughtShape* Live = ResolveDraughtShape(Store, Identifier))
            {
                Live->TintIndex        = Tint;
                Live->FolderIdentifier = Folder;
            }
...
bool PartitionShapeSegment(DraughtShapeStore& Store, uint32_t Identifier, ImVec2 CursorMm, float ToleranceMm)
{
    ...
    if (Count < 2)
    {
        DetachDraughtShape(Store, Identifier);
        return true;
    }
    const DraughtShapeCategory Category = Shape->Category;
    ...
    if ((Category == DraughtShapeCategory::Circle || Category == DraughtShapeCategory::Arc) && ArcRadius > 1e-5f)
    {
        ...
        for (const std::pair<float, float>& Span : Spans)
        {
            ...
            const float AngleFrom = (Category == DraughtShapeCategory::Circle)
                                    ? (Span.first  / Total) * TwoPi
                                    : ArcStart + (Span.first  / Total) * ArcSweep;
            const float AngleTo   = (Category == DraughtShapeCategory::Circle)
        ...
        AppendDraughtEdit(Store, Store.Selected, RoundLabel, "scissors");
        return true;
    }
    ...
    {
        const uint32_t Fresh = AppendDraughtShape(Store, DraughtShapeCategory::Polyline, Survivors[Index]);
        if (Fresh != 0)
        {
            if (DraughtShape* Live = ResolveDraughtShape(Store, Fresh))
            {
                Live->TintIndex        = Tint;
                Live->FolderIdentifier = Folder;
            }
    ...
    AppendDraughtEdit(Store, Store.Selected, LogLabel, "scissors");
    return true;
}
...
bool CutShapeAtPoint(DraughtShapeStore& Store, uint32_t Identifier, ImVec2 CursorMm, float ToleranceMm)
{
    ...
    if (Category == DraughtShapeCategory::Circle && ArcRadius > 1e-5f)
    {
        ...
        DetachDraughtShape(Store, Identifier);   // replaced by its opened arc (never silently dropped — logged below)
        ...
        AppendDraughtEdit(Store, Opened, LogLabel, "scissors");
        return true;
    }
    if (Category == DraughtShapeCategory::Arc && ArcRadius > 1e-5f)
    {
        const ImVec2 CutPoint = PointAtArcLength(W, Closed, CumulativeLength, Total, CutArc);
    ...
    {
        const uint32_t Fresh = AppendDraughtShape(Store, DraughtShapeCategory::Polyline, Pieces[Index]);
        if (Fresh != 0)
        {
            if (DraughtShape* Live = ResolveDraughtShape(Store, Fresh))
            {
...
bool AssembleOpenShapes(DraughtShapeStore& Store, float WeldToleranceMm)
{
    ...
    DraughtShape* Live = ResolveDraughtShape(Store, KeepIdentifier);
    if (Live == nullptr)
        return false;
    const uint32_t Tint   = Live->TintIndex;
    const uint32_t Folder = Live->FolderIdentifier;
    ...
    for (size_t Index = 1; Index < Welded.size(); ++Index)
        DetachDraughtShape(Store, Welded[Index]);

    Live = ResolveDraughtShape(Store, KeepIdentifier);   // re-resolve — Detach may have reallocated the vector
    if (Live == nullptr)
        return false;
    *Live = ConstructDraughtShape(ClosedLoop ? DraughtShapeCategory::Profile : DraughtShapeCategory::Polyline, Path);
    ...
    AppendDraughtEdit(Store, KeepIdentifier, LogLabel, ClosedLoop ? "vector-square" : "spline");
    return true;
}
