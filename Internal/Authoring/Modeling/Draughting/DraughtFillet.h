/*==============================================================================================================================================
                                                               DRAUGHTFILLET.H
==============================================================================================================================================*/
// 🧩 The draughting workspace's analytic corner Fillet / Chamfer engine — the exact-geometry heart of the Plasticity `B` tool. Given ONE
...
//    sweep angle) — the display polyline is a background tessellation, never a dotted overlay. A safe-limit clamp keeps the setback within
//    both leg lengths (Plasticity's max-radius behaviour). Pure CPU + analytic; the modal (DraughtFilletModal) drives the live drag over
//    this, and the store's ConstructDraughtShape re-solves the rebuilt Points so the round-family scalars stay in lockstep.

#pragma once
#ifndef FRONTIER_AUTHORING_MODELING_DRAUGHTING_DRAUGHTFILLET_H
#define FRONTIER_AUTHORING_MODELING_DRAUGHTING_DRAUGHTFILLET_H
...
// 📝 Which corner edit the `B` tool is committing. Fillet rounds the corner into a tangent arc (drag-positive); Chamfer cuts it with a
//    straight setback edge (drag-negative). One tool, two outcomes chosen by the drag sign — the Plasticity idiom. Named .Category per the skills.
enum class DraughtCornerCategory
{
...
// 📝 The resolved geometry of a corner the tool is about to round / cut, computed once from the corner triple + the requested magnitude.
//    Resolved is false for a degenerate corner (collinear legs, a zero leg, or a non-positive magnitude) — the caller then leaves the shape
...
//    fillet Centre / Radius / StartAngle / SweepAngle carry the exact arc, matching DraughtShape's own arc scalars.
struct DraughtCornerSolution
{
...
DraughtCornerSolution SolveCornerEdit(const DraughtShape&    Shape,
                                      int                    Index,
                                      float                  Magnitude,
                                      DraughtCornerCategory  Category);
...
float ResolveCornerSafeLimit(const DraughtShape& Shape, int Index);
...
// point with the two tangent points, splicing the arc span for a closed Profile / open Polyline), re-solve via ConstructDraughtShape,
...
// exact — the view flattens it at the curve-resolution budget for display only.
bool FilletShapeCorner(DraughtShapeStore& Store, uint32_t Identifier, int Index, float Magnitude);
...
// tangent points (a straight edge between them), re-solve via ConstructDraughtShape, record a History edit ("Chamfered <Title>"), and
// re-select. No-op returning false for a degenerate corner / absent shape.
bool ChamferShapeCorner(DraughtShapeStore& Store, uint32_t Identifier, int Index, float Magnitude);
...
void EvaluateArcParametricSnaps(const DraughtCornerSolution& Solution, std::vector<ImVec2>& Out);

} // namespace Frontier

#endif   // FRONTIER_AUTHORING_MODELING_DRAUGHTING_DRAUGHTFILLET_H
