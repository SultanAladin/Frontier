/*============================================================================================================================================
                                                              ADJACENCYINDEX.H
============================================================================================================================================*/
// 🧩 The lightweight derived adjacency cache the overlays + CPU picker read — the engine-named port of the reference
...
//    (which faces meet at each edge — 1 = boundary, 2 = interior). Faces stay ORIGINAL polygons (tris / quads / N-gons); edges
//    are the N loop edges (consecutive corner pairs, wrap-around) — no fan diagonals, so a quad reports four real edges.
// 📝 Edges are keyed by a PACKED uint64 ordered pair (min corner in the high 32 bits, max in the low 32) rather than the
//    prototype's "min_max" string — no per-edge allocation, and the key is a plain map lookup. EdgeVertices recovers the two
//    endpoints from a key; EdgeFaces lists the incident faces; FaceEdges[f] lists face f's loop-edge keys in winding order.
//    A half-edge structure (TopologyStructure) is a SEPARATE, heavier edit cache built only in edit mode — this index needs no
//    half-edges to answer visualization + picking queries, exactly as the prototype's TOPO did.

#pragma once
#ifndef FRONTIER_AUTHORING_MODELING_ADJACENCY_ADJACENCYINDEX_H
#define FRONTIER_AUTHORING_MODELING_ADJACENCY_ADJACENCYINDEX_H
...
struct PolygonCluster;   // 📝 forward-declared; the .cpp reads its face stream + positions (Rendering-free, cheap include)

//------------------------------------------------------------------------------------------------------------------------
//                                                            STRUCTS
//------------------------------------------------------------------------------------------------------------------------

// 📝 One fan triangle mapped back to its incident face — the flat raster/pick list the prototype called TOPO.tris. The three
...
// 📝 The two endpoint vertex indices of one edge, recovered from its packed key. A plain movable value type (a raw C-array
//    member cannot be an unordered_map value — it is neither assignable nor movable), so the edge maps store this. The key
...
// 📝 The whole derived adjacency cache for one cluster. Parallel arrays indexed by face ordinal (FaceVertexLoops …
...
// equivalent of the prototype's edgeKey("min_max") — order-independent, allocation-free.
...
#endif   // FRONTIER_AUTHORING_MODELING_ADJACENCY_ADJACENCYINDEX_H
