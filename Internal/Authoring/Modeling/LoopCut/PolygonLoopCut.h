/*============================================================================================================================================
                                                               POLYGONLOOPCUT.H
============================================================================================================================================*/
// 🧩 The Blender-standard loop-cut kernel for the authoring PolygonCluster: threading the quad ring the seed edge belongs to,
...
//    a non-quad — where it terminates cleanly. The new loop vertices are created at their EVEN rest position (Slide 0) so the
...
#pragma once
#ifndef FRONTIER_AUTHORING_MODELING_LOOPCUT_POLYGONLOOPCUT_H
#define FRONTIER_AUTHORING_MODELING_LOOPCUT_POLYGONLOOPCUT_H
...
struct PolygonCluster;   // 📝 forward-declared — the kernel reads its face stream + positions and rewrites both in place.

//------------------------------------------------------------------------------------------------------------------------
//                                                            STRUCTS
//------------------------------------------------------------------------------------------------------------------------

// 📝 One appended loop vertex and the ring-edge axis it slides along while the caller drives the interactive offset. Direction
...
//    OriginPosition + Direction * Slide — the same origin-snapshot rule Extrude / the drag gizmo use.
...
// 📝 One segment of the loop-cut PREVIEW line: the two ring-edge cut points of a single crossed quad at one cut level (object
//    space). The whole preview is the union of these short segments across every quad the ring crosses at every cut level —
...
// 📝 The whole outcome of a loop-cut insertion: the appended loop vertices + how to drive the live slide, plus the inserted
//    loop as the follow-on selection (LoopVertices → the new vertex selection, LoopEdges → the new edge selection, so the
...
// vertex indices — the picked edge key's two endpoints). CutCount is clamped to >= 1; the cuts sit at parameters i/(CutCount+1)
...
#endif   // FRONTIER_AUTHORING_MODELING_LOOPCUT_POLYGONLOOPCUT_H
