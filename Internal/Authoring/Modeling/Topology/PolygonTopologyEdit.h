...
============================================================================================================================================*/
// 🧩 The Blender-standard topology-removal + hole kernels for the authoring PolygonCluster: Delete (leave a hole), Dissolve (merge
...
//    boundary loops). All four are SINGLE-SHOT — they mutate the cluster's face stream + vertex field once and report the follow-on
...
#pragma once
#ifndef FRONTIER_AUTHORING_MODELING_TOPOLOGY_POLYGONTOPOLOGYEDIT_H
#define FRONTIER_AUTHORING_MODELING_TOPOLOGY_POLYGONTOPOLOGYEDIT_H
...
struct PolygonCluster;   // 📝 forward-declared — the kernels mutate its face stream + positions; the .cpp pulls the full header.
struct AdjacencyIndex;   // 📝 forward-declared — read the base loops / normals / edge incidence before any mutation.
struct SelectionState;   // 📝 forward-declared — Delete / Dissolve read the active-mode set; the .cpp pulls ComponentSelection.h.

//------------------------------------------------------------------------------------------------------------------------
//                                                          ENUMERATIONS
//------------------------------------------------------------------------------------------------------------------------

// 📝 Which component set of the SelectionState a Delete / Dissolve reads (mirrors the active SelectionMode). `.Category` suffix per
...
// 📝 The whole outcome of one topology edit: what to re-select afterwards (shaped like ExtrudeOutcome's cap sets). Only the sets an
//    op produces are filled — Delete fills the surviving hole rim (ResultVertices), Fill / Bridge fill ResultFaces with the new
...
// surviving face still uses, compact the rest — ResultVertices is the removed region's rim. Edge: drop both faces incident to each
// selected edge, then drop any vertex left with no surviving incidence — a true hole. Vertex: drop every face touching a selected
...
#endif   // FRONTIER_AUTHORING_MODELING_TOPOLOGY_POLYGONTOPOLOGYEDIT_H
