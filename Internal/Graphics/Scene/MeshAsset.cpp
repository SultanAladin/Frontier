...
void ReportMeshAsset(const char* MessageText, const char* Detail)
{
    std::fprintf(stderr, "[MeshAsset] %s: %s\n", MessageText, Detail ? Detail : "");
}
...
bool LoadReferenceMeshAsset(const char* FilePath, RenderVertexStream& Result)
{
    ...
    if (Text.empty())
    {
        ReportMeshAsset("file read failed or empty", FilePath);
        return false;
    }
    ...
    {
        ReportMeshAsset("missing vertexCount / triCount", FilePath);
        return false;
    }
    ...
    {
        ReportMeshAsset("missing positions / indices array", FilePath);
        return false;
    }

    if (Positions.size() != (size_t)VertexCount * 3 || Indices.size() != (size_t)TriangleCount * 3)
    {
        ReportMeshAsset("array length does not match declared counts", FilePath);
        return false;
    }
    ...
    for (size_t IndexIterator = 0; IndexIterator < Indices.size(); ++IndexIterator)
    {
        ...
        {
            ReportMeshAsset("index out of range", FilePath);
            Result = RenderVertexStream{};
            return false;
        }
